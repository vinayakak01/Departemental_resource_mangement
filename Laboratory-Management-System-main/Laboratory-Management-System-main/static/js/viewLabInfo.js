    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            for (var i=0; i<obj.length; i++) 
            {
                ...
                var section = "<section id ='";
                section += inner.pk;
                section += "' class= 'labinfo'> <div>  __Name____:&emsp; <input id='name' class='field' type='text' value='";
                section += inner.fields.labName;
                section += "' readonly size='10'></div> <div>  __ID______:&emsp; <input id='ID' class='field' type='number' value='";
                section += inner.fields.labID;
                section += "' readonly size='4'> </div> <div>Floor_NO:&emsp; <input id='building' class='field' type='number' value='";
                section += inner.fields.bNum;
                section += "' readonly size='4'></div> <div> No_of_PC:&emsp; <input id='floor' class='field' type='number' value='";
                section += inner.fields.fNum;
                section += "' readonly size='4'></div> <div>projectors:&emsp; <input id='pcNum' class='field' type='number' value='";
                section += inner.fields.pcNum;
                section += "' readonly size='4'></div> <input id = '";
                section += inner.pk;
                section += "' onclick=\"edit('";
                section += inner.pk;
                section += "')\" class='button edit' title ='edit' src = '../static/images/edit.png' type='image'/> <div> No_of_chairs:&emsp; <input id='chairs' class='field' type='number' value='";
                section += inner.fields.chairNum;
                section += "' readonly size='4'></div> <div> No_of_Printers:&emsp; <input id='capacity'  class='field' type='number' value='";
                section += inner.fields.cap;
                section += "' readonly size='4'></div> <input id = '";
                section += inner.pk;
                section += "' onclick=\"reportLab('";
